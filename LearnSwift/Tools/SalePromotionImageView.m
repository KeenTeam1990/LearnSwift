...
#define KTSCREENWIGTH  [UIScreen mainScreen].bounds.size.width
#define KTSCRENHIGHT   [UIScreen mainScreen].bounds.size.height
...
-(void)showInView:(UIViewController *)viewController{
    ...
    [UIView animateWithDuration:1.3 delay:1.0 usingSpringWithDamping:0.65 initialSpringVelocity:0.5 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        rect.origin.y = KTSCRENHIGHT/2-50;
        WeakSelf.frame = rect;
...
-(void)moveToFinalLocation{
    ...
    CGFloat speed = KTSCREENWIGTH/1.0f;
    ...
    }else if (midX <= KTSCREENWIGTH/2){
        //贴到左边
        animationTime = minX/speed;
        self.locationType = SalePromotionImageViewLocationType_Left;
    }else{
        //贴到右边
        animationTime = (KTSCREENWIGTH-maxX)/speed;
...
-(void)setImageUrl:(NSString *)imageUrl{
    if (imageUrl == nil || [imageUrl isEqual:[NSNull null]] || imageUrl.length == 0 || [imageUrl stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]].length == 0) {
        return ;
    }
    _imageUrl = imageUrl;
}
...
-(void)setLocationType:(SalePromotionImageViewLocationType)locationType{
    ...
    switch (locationType) {
        ...
        case SalePromotionImageViewLocationType_Center:
            ...
            break;
        case SalePromotionImageViewLocationType_RIght:
            pieceRect.origin.x = KTSCREENWIGTH - pieceRect.size.width;
            break;
    }
