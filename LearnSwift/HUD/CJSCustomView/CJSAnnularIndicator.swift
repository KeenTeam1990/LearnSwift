//
//  CJSAnnularIndicator.swift
//  CJSProgressHUD
//
//  Created by mtrcjs on 2016/12/9.
//  Copyright © 2016年 CJS. All rights reserved.
//

import UIKit

class CJSAnnularIndicator: UIView {
    ...
    fileprivate func setProgress(progress: CGFloat) {
        ...
        if 0 <= progress && progress <= 100.0 {
            ...
            let startAngle: CGFloat = CGFloat(M_PI/2) - CGFloat(M_PI)
            let endAngle = CGFloat(M_PI * 1.5)
            let arcCenter = CGPoint(x: frame.size.width/2,
                                    y: frame.size.height/2)
